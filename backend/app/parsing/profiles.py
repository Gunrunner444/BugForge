...
JAVASCRIPT = LanguageProfile(
    ...
    function_patterns=(
        ...
        r"^\s*(?:async\s+)?(?:get|set|static\s+)?([A-Za-z_$][\w$]*)\s*\([^)]*\)\s*\{",
    ),
...
TYPESCRIPT = LanguageProfile(
    ...
    + (
        ...
        r"(?:public|private|protected|readonly|static|\s)*\s*([A-Za-z_$][\w$]*)\s*\([^)]*\)\s*:\s*[\w<>,\[\]\s|]+\s*\{",
    ),
...
RUBY = LanguageProfile(
    ...
    redirect_sinks=(r"redirect_to", r"redirect_back"),
...
C = LanguageProfile(
    ...
    function_patterns=(r"^[\w\s\*]+\s+([A-Za-z_][\w]*)\s*\(",),
    class_patterns=(),
...
JAVA = LanguageProfile(
    ...
    function_patterns=(
        r"(?:public|private|protected|static|final|native|synchronized|\s)+\s+[\w<>\[\]]+\s+([A-Za-z_][\w]*)\s*\(",
    ),
...
PHP = LanguageProfile(
    ...
    assignment_patterns=(r"\$([A-Za-z_][\w]*)\s*=\s*(.+)$",),
    bare_call_keywords=("echo", "print", "include", "require", "include_once", "require_once"),
    ...
    xss_sinks=(r"\becho\b", r"\bprint\b", r"<\?="),
