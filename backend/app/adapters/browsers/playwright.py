"""Playwright browser adapter. Every navigation goes through ScopeGuard."""
...
from app.security_testing.secrets import redact_mapping
...
class PlaywrightBrowserAdapter(BrowserAdapter):
    ...
    async def _navigate(self, url: str) -> None:
        if not self.is_available():
            ...
            self._last = BrowserEvidence(url=url, title=host, history=tuple(self._history))
            return
    ...
    async def capture(self) -> BrowserEvidence:
        ...
        if self.policy.capture_storage:
            ...
            local_meta = tuple(str(k) for k in local)
            session_meta = tuple(str(k) for k in session)
            redact_mapping({k: "x" for k in local_meta})
        evidence = BrowserEvidence(
            url=url,
            title=title,
            dom=dom,
